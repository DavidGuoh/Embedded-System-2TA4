...
__IO uint16_t TIM3_CCR1_Val=0, TIM3_CCR2_Val=333,TIM3_CCR3_Val=666,TIM3_CCR4_Val=1000;  // for dynamically set the pulse width  for the four channels. 
...
__IO uint8_t B1Pressed=0;
__IO uint8_t B2Pressed=0;
...
int main(void){
	...
	while(1) {		
		...
		ctemp = HAL_ADC_GetValue(&AdcHandle)*0.02442002; //Convert ADC value to degree oC
		LCD_DisplayFloat(9, 10, ctemp, 2); 
		if(ctemp < setPoint){ //If temperature lower than setPoint
			HAL_TIM_PWM_Stop(&Tim3_Handle,TIM_CHANNEL_2); //Stop the PWM output
			HAL_TIM_Base_Stop(&Tim3_Handle); //Stop the Timer
			Tim3_OCInitStructure.Pulse=0; //Set Pulse as 0
			LCD_DisplayString(13, 0, (uint8_t *)"Speed"); 
			LCD_DisplayInt(13, 10, 0);
			LCD_DisplayString(13, 12, (uint8_t *)"%");
			if(clean == 1){
			...
			HAL_TIM_PWM_Start(&Tim3_Handle,TIM_CHANNEL_2); //Start the PWM

			double pulsesize = 500+(ctemp-setPoint)*100; //Set the pules size with respect to the temperature above setPoint
			
			if (pulsesize > 1000){
				Tim3_OCInitStructure.Pulse = 1000; //if temperature higher than a certain value then set pulsesize == 1000
			}
			else{
				Tim3_OCInitStructure.Pulse = pulsesize;
			}
			
			
			LCD_DisplayString(13, 0, (uint8_t *)"Speed");
			LCD_DisplayFloat(13, 10, Tim3_OCInitStructure.Pulse/10, 2);
			LCD_DisplayString(13, 15, (uint8_t *)"%");
			clean = 1;
		}
		__HAL_TIM_SET_COMPARE(&Tim3_Handle, TIM_CHANNEL_2, Tim3_OCInitStructure.Pulse); //Set the Pulse
		/*if (B1Pressed == 1){
			OC_Count = 0;
			increment = 0;
			while(!HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_2)){			// while button is pressed
				if (increment == 1){
					increment = 0;
			...
			OC_Count = 0;
			increment = 0;
			while(!HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_1)){			// while button is pressed
				if (increment == 1){
					increment = 0;
					setPoint= setPoint+0.5;
					LCD_DisplayString(10,10,(uint8_t *) "     ");
	...
	if(GPIO_Pin == GPIO_PIN_1)
  {
	...
	if(GPIO_Pin == GPIO_PIN_2)
	{
		...
		while(!HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_2)){ // while button is pressed
			if((HAL_GetTick()- holdstart) > 500){ // if held more than 0.5s
				holdstart = HAL_GetTick();
				setPoint -= 0.5; //Set point value - 0.5
				LCD_DisplayString(10,10,(uint8_t *) "     ");
				LCD_DisplayFloat(10, 10, setPoint, 2);
			}
		}
